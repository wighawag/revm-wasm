...
- The gap this leaves is deliberate: revm's `disable_fee_charge` is not exposed. It has no demonstrated consumer, and would be another `optional_*` feature in the shipped build. Add it the same way, with a use case attached. (`disable_eip3607`, which this ADR originally listed as a gap, is now exposed as `disableEip3607` on `ExecuteOptions`, flag bit 7 — same shape, same discipline, same default-off guarantee. See `.changeset/disable-eip3607.md`.)
